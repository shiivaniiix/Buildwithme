...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    // Validate messages array (optional, but if provided must be valid)
    if (messages && !Array.isArray(messages)) {
      return NextResponse.json(
        { error: "messages must be an array if provided" },
        { status: 400 }
      );
    }
...
- Keep responses conversational and natural`;
    ...
    // Build project context (file summaries, technologies, structure summary)
...
${JSON.stringify({ nodes: graph.nodes.length, edges: graph.edges.length, technologies: graph.technologies }, null, 2)}`;
    ...
    if (fileSummaries && typeof fileSummaries === "object" && Object.keys(fileSummaries).length > 0) {
      ...
      for (const [filePath, content] of Object.entries(fileSummaries)) {
        // Limit content length to keep token usage reasonable (max 1500 chars per file)
    ...
    console.error("Error in /api/codegraph/chat:", error);
    return NextResponse.json(
